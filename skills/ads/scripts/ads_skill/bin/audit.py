...
def audit_campaign(client: Any, customer_id: str, camp: Any, banned: list[str],
    ...
    for r in rows:
        ...
        ads_out.append({
            ...
            # Full asset text (not just counts) so /ads:update can preserve good copy
...
# ---------------------------------------------------------------------------
# Search-term layer — the same wasted-spend / scale-up signal /ads:report derives,
# pulled into the audit so negatives can be chosen from real query data (not
# guessed). negatives_to_add = terms that spent without converting (→ /ads:update
# negative keywords); keywords_to_promote = converting terms not yet keywords
# (→ /ads:update positive-keyword adds). Pure logic lives in lib/cluster.
# ---------------------------------------------------------------------------
