...
| ------------------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------- |
| **Promote** (add keyword) | On-theme and converting (or high CTR) — reveals intent you hadn't covered          | Classify by intent, add to the right ad group |
| **Demote** (add negative) | Off-theme, wrong intent, brand leakage, or too generic for the ad group it landed in | Classify into the right negative list |
...
Work top-down by cost, not row count. Stop when remaining rows don't move the account.
