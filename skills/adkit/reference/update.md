...
- **`bidding`** set a campaign's **bid strategy**, in either direction — `campaignId`, `strategy` (`maximize-clicks`/`maximize-conversions`), and an optional `cpcBidCeilingMicros` (valid only with `maximize-clicks`, same rule `/adkit create` briefs already enforce). Graduating up (`maximize-clicks` → `maximize-conversions`) is unguarded. Downgrading (`maximize-conversions` → `maximize-clicks`) — the fix for a `cold_start_throttle` audit finding, where Smart Bidding has too little conversion volume to bid well — is **refused** when the campaign has **≥30 conversions in the trailing 30 days**, unless the entry sets `acknowledgeStrategyDowngrade: true`; this guards against silently undoing a campaign's proven Smart Bidding graduation. A `cpcBidCeilingMicros` below the campaign's own trailing-30-day average CPC prints a non-blocking `WARNING:` (it would likely starve the campaign of traffic) but still applies.
