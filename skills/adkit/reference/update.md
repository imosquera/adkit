...
- **`addRsa`** add a **second, distinct RSA to an ad group live with fewer than 2 non-REMOVED RSAs** — the fix for an audit `rsa_count_mismatch` finding (a legacy ad group, or one that predates the `responsiveSearchAd` → `responsiveSearchAds` migration). `adGroupId` is digits-only; `headlines`/`descriptions` are the same **15/4 bare-string** ergonomics as `rewrites`/`adGroups`' embedded RSAs, validated by the **identical RSA rules** `/adkit create` enforces (author the copy per §1: keyword in ≥3 headlines, a genuinely distinct angle from the ad group's existing RSA, no pins). `finalUrl` is optional — it **defaults to the ad group's existing live RSA's `finalUrl`** when the ad group has exactly 1 live RSA; when the ad group has 0 live RSAs, omitting `finalUrl` is a **validation error** (nothing to default from). `path1`/`path2` follow the same display-path rules `rewrites` already enforces. The new RSA is created **PAUSED**, same convention as every other RSA this skill creates. **Idempotent** — an ad group already at 2 (or more) live RSAs is reported **skipped**, never given a 3rd; re-running the same plan after it's already applied is a safe no-op.
