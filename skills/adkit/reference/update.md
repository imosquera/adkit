...
- **`audiences`** attach or detach **audience segments on an ad group** (`AdGroupCriterion`) — the lever for adding an existing remarketing/customer-match/in-market/affinity/custom-intent segment to a live ad group, or removing one. `add`/`remove` items are bare numeric audience IDs (from `ads.sh audiences list` — see `reference/audiences.md`). Unlike `keywords`, this lever is **idempotent in both directions**: re-adding an already-attached segment is silently skipped (no duplicate criterion, no error), and re-removing an already-absent one is also silently a no-op — never rejected. On a plain Search campaign the attached segment behaves as an observation/signal only (never a targeting restriction); on a `"display-remarketing"` campaign the same criterion is true targeting.
