...
3. Under `### Keywords`, create a subsection for **each intent tier that has at least one kept keyword**, in this canonical order (**omit any tier the Keyword Planner returned no kept keywords for** — see the thin-source rule in #5; do not emit an empty tier heading and do not invent keywords to fill one):
...
5. Each present intent tier then lists keyword bullets. **Aim for ~100 keywords total to start** when the source can support it, spread across whichever intent tiers the returned set actually populates (roughly 20-30 per tier is the target for a rich niche) — a campaign wants ~25+ keywords to compete (what `ads:audit` flags as `keywords_under`), and starting near 100 gives real reach and room to prune to the winners. **The real floor is the kept set the Keyword Planner actually returned, not a fixed count.** Every keyword must be a Keyword-Planner-returned candidate that survived screening (steps 8–9) — so **do not fabricate keywords, split near-duplicates, or hand-add zero-volume phrasings to hit ~100, a per-tier count, or a four-tier spread** (that violates step 8 and Keyword Research Guidance item 8). A genuinely thin, single-intent niche legitimately yields far fewer keywords concentrated in one or two tiers; write exactly what the kept set supports and let the missing tiers be absent (#3). There is **no per-tier minimum** — a present tier holds however many kept keywords classified into it, and tiers with zero are omitted, not padded.
...
12. Cap each tier at 8–16 bullets; truncate the tail if more were classified there. A tier with few kept keywords is fine — write only what genuinely classified there and **omit the tier entirely if zero keywords landed in it** (#3). Never redistribute a keyword into a tier its true buying-cycle temperature doesn't match, and never fabricate phrases, to prop up a thin tier — a lopsided or single-tier spread is the correct output for a thin, single-intent niche.
...
17. Re-read the processed file and verify:
   ...
   - **at least one** intent tier subsection exists, and every tier that is present appears in canonical order (Informational, Navigational, Commercial, Transactional) — absent tiers are expected when the kept set didn't populate them; a tier is present **iff** it has ≥1 kept keyword,
   - **no tier was padded**: every keyword bullet traces to a Keyword-Planner-returned candidate (verbatim `bullet_text`), with no fabricated or near-duplicate phrases added to reach a count,
   ...
   - each present tier subsection has a single `> Default offer:` line directly under its heading,
   - each present tier subsection has at least one keyword bullet (no empty tier headings),
