...
**RSA count per ad group** — every ad group must publish exactly **2 RSAs** (`/adkit create`'s hard-coded target; a serving fallback plus angle-level diversity, not reworded twins — issue #175). The count includes both ENABLED and PAUSED ads (only REMOVED is excluded) — deliberately, since `/adkit create` always publishes RSAs PAUSED, so an ENABLED-only count would false-flag every freshly-published campaign. A campaign-level `rsa_count_mismatch` finding names any ad group whose RSA count isn't exactly 2 (a legacy 1-RSA ad group, or 3+ from a manual UI edit), with `detail` reading `ad group '<name>': <count>/2 RSAs`. **Under 2 is fixable via `/adkit update`'s `addRsa` lever** — author an `addRsa` block keyed by the ad group's `adGroupId` and apply it; a new, distinct-angle RSA is created **PAUSED**, idempotently (re-running once the group is at 2 is a no-op skip, and it never creates a 3rd). **3+ (an over-count) is still not fixable** — there is no remove-RSA operation, so an over-count still requires manually removing the extra distinct-angle RSA directly in the Ads UI.
