...
- **One keyword theme per ad group.** Keywords are grouped by **semantic theme** — the 3–6 `### Keyword Themes` `/adkit gtm` authors (step 15c), e.g. *salon software*, *barber/stylist*, *free intent* — *not* one-keyword-per-group. `### Keywords` is a flat, kept keyword list with no separate classification; the Keyword Themes are the only grouping. Google's close-variant matching and Smart Bidding made micro-SKAGs obsolete: theme groups consolidate conversion data so the ML optimizes faster, and ad copy can mirror the theme for a better Quality Score.
...
- **AI Max** (`campaign.aiMax`, default **on**) reaches *beyond* your literal keywords via broad-match tech + landing-page matching. With STAGs it pairs well: the theme sets intent, AI Max finds the long-tail you can't enumerate, negatives keep it on-theme. Search-term matching is applied **per ad group** via `adGroups[].aiMax` (default **off**) — so a campaign runs AI Max while each ad group stays on strict keyword matching until it opts in.
...
| `campaign.networkSettings` | Default `"search-partners-display"` — serves on Google search **plus** search partner sites; the Display Network is disabled. Set `"search-only"` to restrict to Google search results only. Set `"display-remarketing"` to additionally enable the Display Network for this campaign (Search Network with Display Select — the campaign still authors Responsive Search Ads via the same pipeline, not a separate Display-creative type) — **only valid when at least one `adGroups[].audienceSegments` entry is present somewhere in the brief**; a `"display-remarketing"` brief with no audience segment anywhere is rejected before any campaign is created. |
| `adGroups[].audienceSegments` | Optional list of `{ audienceId: <numeric Google Ads audience ID> }` — attaches existing audience segments (in-market, affinity, custom-intent/custom-audience, or user-list/remarketing/customer-match; see `reference/audiences.md`'s `list` command) to this ad group via `AdGroupCriterion`. Defaults to `[]` — omitting it is unchanged from before this field existed. On a plain Search campaign this behaves as an observation/signal (bid/report visibility only, never a targeting restriction); on a `"display-remarketing"` campaign the same criterion is true targeting. |
...
| `campaign.priceAsset` | Optional campaign-level price asset: 3–8 offerings with a ≤25-character header/description, positive `priceMicros`, and an https `finalUrl`. **Omitted from the scaffold** (a brief validates without it) — a ready-to-uncomment template is appended, commented out, at the end of the scaffolded brief. |
| `campaign.structuredSnippet` | Optional campaign-level structured snippet: a supported header and 3–10 distinct values (≤25 characters each). **Omitted from the scaffold** — see the commented `OPTIONAL ADD-ONS` template at the end of the brief. |
