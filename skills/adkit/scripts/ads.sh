...
#   subcommands: init | preflight | create | keyword-ideas | research | report | audit | update | render-yaml | bootstrap-secrets | audiences
...
if [ "$#" -lt 1 ]; then
  echo "usage: ads.sh <init|preflight|create|keyword-ideas|research|report|audit|update|render-yaml|bootstrap-secrets|audiences> [args...]" >&2
  exit 1
fi
...
case "$cmd" in
  ...
  init|preflight|create|keyword-ideas|research|report|audit|render-yaml|bootstrap-secrets|audiences) mod="$cmd" ;;
