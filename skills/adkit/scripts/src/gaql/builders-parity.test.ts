...
const CASES: ReadonlyArray<[string, string]> = [
  ...
  [
    ...
    "SELECT campaign.id, campaign.name, segments.date, campaign.status, metrics.cost_micros, metrics.impressions, metrics.clicks, metrics.ctr, metrics.average_cpc, metrics.conversions, metrics.cost_per_conversion FROM campaign WHERE campaign.status = 'ENABLED' AND segments.date BETWEEN '2026-06-08' AND '2026-06-21' ORDER BY segments.date",
  ],
  [
  ...
  [
    ...
    "SELECT campaign.id, ad_group.id, ad_group.name, ad_group_criterion.keyword.text FROM ad_group_criterion WHERE campaign.id IN (12345,67890) AND ad_group_criterion.type = 'KEYWORD' AND ad_group_criterion.status != 'REMOVED'",
  ],
  [
