...
An operator who has learned the `--customer <id>` convention from `audit`, `create`, and `update` runs `adkit report --customer 8911925499 --days 30` expecting the report to run against account `8911925499`. Today that flag is ignored and the run fails; the operator should get the same behavior as every other subcommand.
