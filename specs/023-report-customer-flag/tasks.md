...
- [ ] T011 Manual end-to-end verification per spec Verify section: from the repo root run `bash ads.sh report --customer 1111111111 --days 30` (expect JSON + dashboard / a real error, never `[object Object]`) and `bash ads.sh report 1111111111` (back-compat). Record the observed output. (depends on T010)
